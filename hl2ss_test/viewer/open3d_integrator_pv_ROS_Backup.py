...
class controler():
    def __init__(self):
        ...
        self.Tx = np.eye(4)
        self.Tx[:3, :3] = rotx #Rotation of the QR code
            ...
            if control.state != None:
                ...
                if control.state.state == 5.:
                    ...
                    if data.pos_x != 0.:
                        ...
                        r = R.from_quat([data.rot_x, data.rot_y, data.rot_z, data.rot_w])
                        T[:3,:3] = r.as_matrix()

                        #Translate the mesh with the T matrix
                        #No transformation
                        control.pcd = control.pcd.transform(np.linalg.inv(T @ control.Tx))
...
# Settings --------------------------------------------------------------------
#Start the nodes for ROS communication
