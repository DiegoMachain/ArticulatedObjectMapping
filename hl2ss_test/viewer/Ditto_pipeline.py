...
def Ditto(src_pcd_list, dst_pcd_list, model, generator, device):
    ...
    print("Path = ", os.path.join(os.getcwd(), 'URDF/Fixed/static.stl'))
    #Save the the mesh as stl
    fixed_static_mesh.export(os.path.join(os.getcwd(), 'URDF/Fixed/static.stl'))
    fixed_mobile_mesh.export(os.path.join(os.getcwd(), 'URDF/Mobile/mobile.stl'))

    static_part.export(os.path.join(os.getcwd(), 'URDF/Fixed/static_original.stl'))
    mobile_part.export(os.path.join(os.getcwd(), 'URDF/Mobile/mobile_original.stl'))

    #We still need to return to the previous space

    #Here we save the URDF model
    saveURDF.save_URDF(pivot_point_pred, joint_axis_pred, os.getcwd())
