...
class manager_2 {
    ...
    ros::NodeHandle r;
    ros::Publisher model_pub = r.advertise<articulation_model_msgs::ModelMsg> ("model_track", 5);
...
void poseCallback(geometry_msgs::Pose pose){
  ...
  if (manag.numb_msgs < manag.initial_datapoints_number + 1){
    manag.generated_poses.push_back(pose);
    manag.numb_msgs +=1;
  }
  if (manag.numb_msgs >= manag.initial_datapoints_number + 1){
    

    manag.sigma_param.name = "sigma_position";
    manag.sigma_param.value = 0.02;
    manag.sigma_param.type = articulation_model_msgs::ParamMsg::PRIOR;
    manag.model_msg.params.push_back(manag.sigma_param);


    manag.model_msg.track = generateMeasurement(manag.generated_poses, 0, manag.initial_datapoints_number + 1);
    ...
    z = generateMeasurement(manag.generated_poses, 0, manag.initial_datapoints_number + 1);
    rotational_instance->addTrack(z);
    prismatic_instance->addTrack(z);
    rigid_instance->addTrack(z);
    std::cout << "measurement taken callBack"<< z << std::endl;
    ...
    pos_x = rigid_instance->getParam("rigid_position.x");
    pos_y = rigid_instance->getParam("rigid_position.y");
    pos_z = rigid_instance->getParam("rigid_position.z");
    model_name = "rigid";

    std::cout << "     "<< model_name << " x = "<< pos_x << "\n \n" << std::endl;
    std::cout << "     "<< model_name << " y = "<< pos_y << "\n \n" << std::endl;
    std::cout << "     "<< model_name << " z = "<< pos_z << "\n \n" << std::endl;

    manag.model_pub.publish(rotational_instance->getModel());

    manag.model_pub.publish(prismatic_instance->getModel());

    manag.model_pub.publish(rigid_instance->getModel());
...
int main(int argc, char** argv)
{
  ...
  manager_2* manag = new manager_2();
  ros::Subscriber sub = n.subscribe("trajectory_pose", 100, poseCallback);
  ...
  for (int i = 0; i < datapoints_number; i++)
  {
    ...
    if (prismatic)
    {
      ...
      pose.position.y = 10 +(static_cast<float> (i)/100.0) + var_nor();
      pose.position.z = 20 + (static_cast<float> (i)/100.0) + var_nor();
    }
    else if (rotational)
    {
      pose.position.x = 2 + cos(static_cast<float> (i) / 100.0) + var_nor();
      pose.position.y = 6 + sin(static_cast<float> (i) / 100.0) + var_nor();
...
//     boost::shared_ptr<RotationalModel> rotational = boost::dynamic_pointer_cast< RotationalModel > (rotational_instance);
...
////    std::cout << "     pos.x = " << rigid->pos_x << std::endl;
    ...
    if (rotational){
      ...
      pos_x = prismatic_instance->getParam("rigid_position.x");
      pos_y = prismatic_instance->getParam("rigid_position.y");
      pos_z = prismatic_instance->getParam("rigid_position.z");
